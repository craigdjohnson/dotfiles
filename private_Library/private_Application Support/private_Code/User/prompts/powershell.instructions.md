...
- **Variable Names:**
  ...
  - Prefer $PSItem for clarity, but note that $_ is still valid and widely used in the PowerShell community; this is a style preference.
...
function Update-ResourceStatus {
    ...
    begin {
        $logger = [CustomLogger]::GetLogger($MyInvocation.MyCommand.Name)
        $logger.verbose('Starting resource status update process')
        $timestamp = Get-Date
    }

    process {
        # Process each resource individually
        $logger.verbose("Processing resource: $Name")
        ...
        $logger.verbose('Resource status update process completed')
    }
}
```
...
- **Message/Logging Streams:**
  - Use `[CustomLogger]` class via `$logger = [CustomLogger]::GetLogger($MyInvocation.MyCommand.Name)` for logging
    - Initialize logging environment via `Initialize-CustomLogging -Path <logPath> -Level Debug`
  - `$logger.verbose($message)` for operational details
  - `$logger.debug($message)` for debugging information
  - `$logger.warning($message)` for warning conditions
  - `$logger.error($message)` for logging errors
...
function Remove-UserAccount {
    ...
    begin {
        $logger = [CustomLogger]::GetLogger($MyInvocation.MyCommand.Name)
        $logger.verbose('Starting user account removal process')
    ...
    process {
        try {
            ...
            if ($Force -or $PSCmdlet.ShouldProcess($Username, $shouldProcessMessage)) {
                $logger.verbose("Removing user account: $Username")

                # Main operation
                Remove-ADUser -Identity $Username -ErrorAction Stop
                $logger.warning("User account '$Username' has been removed")
            }
        ...
        $logger.verbose('User account removal process completed')
    }
}
```
...
- **Comment-Based Help:** Include comment-based help for any public-facing function or cmdlet. Inside the function, add a `<# ... #>` help comment with at least:
  ...
  - `.LINK` Related URLs, documentation, or functions
...
function New-Resource {
    ...
    begin {
        $logger = [CustomLogger]::GetLogger($MyInvocation.MyCommand.Name)
        $logger.verbose('Starting resource creation process')
    }
    ...
    process {
        ...
        $logger.verbose('Completed resource creation process')
    }
}
```
